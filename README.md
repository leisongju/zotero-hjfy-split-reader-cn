...
这个仓库是基于 [Infinity4B/zotero-hjfy-split-reader](https://github.com/Infinity4B/zotero-hjfy-split-reader) 的改版。改版重点是让插件在 Zotero 条目元数据不完整时更稳：自动补找 arXiv ID，绕开 `hjfy.top` 的 `arxivInfo` 接口偶发失败，下载成功后把译文 PDF 挂回原论文条目。
...
- 对 `hjfy.top/api/arxivInfo` 增加超时和失败兜底，避免接口异常时整个流程卡死。
- 轮询 HJFY 翻译状态时显示进度窗口，减少“像是卡住了”的误解。
- 支持 HJFY 返回相对路径 PDF URL 的情况。
...
5. GitHub Actions 会根据 tag 生成 Release 和 `.xpi`。如果你不用 Actions，也可以手动上传 `.scaffold/build/hjfy-split-reader.xpi`。

如果你把仓库发布到其他用户名或组织下面，请同步修改 `package.json` 里的 `repository`、`bugs`、`homepage`，以及 `src/modules/hjfySplit.ts` 里的 User-Agent 链接。
